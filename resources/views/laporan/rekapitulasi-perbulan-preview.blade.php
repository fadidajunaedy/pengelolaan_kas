...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
    <ol class="list-reset flex">
        ...
        <li class="text-neutral-500 dark:text-neutral-400">{{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('F Y') }}</li>
    </ol>
  <form action="{{ url('/generate-rekapitulasi-perbulan') }}" method="get">
    @csrf
    @php($bln = \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->format('F'))
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Penerimaan Kas Bulan {{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
      <p class="card-header-title">
          <span class="icon"><i class="mdi mdi-cash"></i></span>
          Pengeluaran Kas Bulan {{ \Carbon\Carbon::parse($dataPengeluaran[0]->tanggal)->translatedFormat('F Y') }}
      </p>
  </header>
  <div class="card-content mb-6">
    <table>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum bayar pada bulan {{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
      <p class="card-header-title">
          <span class="icon"><i class="mdi mdi-cash"></i></span>
          Anggota yang belum melakukan Pengeluaran pada Bulan {{ \Carbon\Carbon::parse($dataPengeluaran[0]->tanggal)->translatedFormat('F Y') }}
      </p>
  </header>
  <div class="card-content">
    <table>
