...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
  ...
  <form action="{{ url('/generate-pengeluaran-pertahun') }}" method="get">
    @csrf
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Pengeluaran Kas Tahun {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum melakukan pengeluaran pada Tahun {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('Y') }}
        </p>
    </header>
