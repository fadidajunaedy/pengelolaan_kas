...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
    <ol class="list-reset flex">
        ...
        <li class="text-neutral-500 dark:text-neutral-400">{{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('d F Y') }}</li>
    </ol>
  <form action="{{ url('/generate-rekapitulasi-pertanggal') }}" method="get">
    @csrf
    @php($tgl = \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->format('Y-m-d'))
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Penerimaan Kas tanggal {{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('d F Y') }}
        </p>
    </header>
    <div class="card-content mb-6">
      <table>
        ...
        <tbody>
          ...
          @endforeach
          <tr>
            <td class="text-center font-bold" colspan="4">Total</td>
    ...
    <header class="card-header">
      <p class="card-header-title">
          <span class="icon"><i class="mdi mdi-cash"></i></span>
          Pengeluaran Kas tanggal {{ \Carbon\Carbon::parse($dataPengeluaran[0]->tanggal)->translatedFormat('d F Y') }}
      </p>
  </header>
  <div class="card-content mb-6">
    <table>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum bayar pada tanggal {{ \Carbon\Carbon::parse($dataPenerimaan[0]->tanggal)->translatedFormat('d F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
      <p class="card-header-title">
          <span class="icon"><i class="mdi mdi-cash"></i></span>
          Anggota yang belum melakukan Pengeluaran pada tanggal {{ \Carbon\Carbon::parse($dataPengeluaran[0]->tanggal)->translatedFormat('d F Y') }}
      </p>
  </header>
  <div class="card-content">
    <table>
