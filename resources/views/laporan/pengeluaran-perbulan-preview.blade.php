...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
    <ol class="list-reset flex">
        ...
        <li class="text-neutral-500 dark:text-neutral-400">{{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('F Y') }}</li>
    </ol>
  <form action="{{ url('/generate-pengeluaran-perbulan') }}" method="get">
    @csrf
    @php($bln = \Carbon\Carbon::parse($data[0]->tanggal)->format('F'))
    @php($th = \Carbon\Carbon::parse($data[0]->tanggal)->Format('Y'))
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Pengeluaran Kas bulan {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum melakukan pengeluaran pada bulan {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
