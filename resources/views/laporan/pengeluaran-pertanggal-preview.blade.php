...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
    <ol class="list-reset flex">
        ...
        <li class="text-neutral-500 dark:text-neutral-400">{{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('d F Y') }}</li>
    </ol>
  <form action="{{ url('/generate-pengeluaran-pertanggal') }}" method="get">
    @csrf
    @php($tgl = \Carbon\Carbon::parse($data[0]->tanggal)->format('Y-m-d'))
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Pengeluaran Kas tanggal {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('d F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum melakukan pengeluaran pada tanggal {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('d F Y') }}
        </p>
    </header>
