...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
    <ol class="list-reset flex">
        ...
        <li class="text-neutral-500 dark:text-neutral-400">Rekapitulasi</li>
    </ol>
</nav>
@include('components.notification')
<div class="card-content">
          <form id="form-get-by-tanggal" action="{{ url('/rekapitulasi-pertanggal-preview') }}" method="get">
            @csrf
        ...
        <hr>
        <form id="form-get-by-bulan" action="{{ url('/rekapitulasi-perbulan-preview') }}" method="get">
            @csrf
            <div class="field flex items-center gap-2">
                ...
                <select name="bulan" class="input basis-1/4">
                    ...
                    @php($bulan = [])
                    @foreach ($tanggalPenerimaan as $item)
        ...
        <hr>
        <form id="form-get-by-tahun" action="{{ url('/rekapitulasi-pertahun-preview') }}" method="get">
            @csrf
            <div class="field flex items-center gap-2">
