...
<nav class="w-full rounded-md flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 mb-4">
  ...
  <form action="{{ url('/generate-penerimaan-perbulan') }}" method="get">
    @csrf
...
<div class="card has-table">
    <header class="card-header">
        <p class="card-header-title">
            ...
            Penerimaan Kas bulan {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
    ...
    <header class="card-header">
        <p class="card-header-title">
            <span class="icon"><i class="mdi mdi-cash"></i></span>
            Anggota yang belum bayar pada bulan {{ \Carbon\Carbon::parse($data[0]->tanggal)->translatedFormat('F Y') }}
        </p>
    </header>
